      ...
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1.5 text-slate-400">
            You have{" "}
            <span className="font-medium text-slate-200">1 assessment assigned</span> by{" "}
            <span className="font-medium text-slate-200">{ASSESSMENT.organization}</span>.
          </p>
        </div>

        {/* Main grid: assessment card + sidebar */}
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-800 p-6 lg:col-span-2">
            <div className="mb-5 flex items-start justify-between gap-4">
              ...
              <Badge dark variant="warning">Not Started</Badge>
            </div>
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              {ASSESSMENT_DETAILS.map((row) => (
                <div
                  key={row.label}
                  className="rounded-lg bg-slate-700/30 px-4 py-3"
                >
          ...
          <div className="flex flex-col gap-4">
              ...
              {CONSENT.confirmed ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">Recorded {CONSENT.date}</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">Must be confirmed before starting.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-800 p-5">
              ...
              <div className="flex items-end justify-between">
                <span className="text-4xl font-bold text-white">{PROGRESS.percentage}%</span>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {DOMAINS.map((domain) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">{domain.items}</p>
                {domain.dimensions && (
                  <p className="mt-0.5 text-xs text-slate-600">{domain.dimensions} dimensions</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mt-6">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-800">
            <div className="divide-y divide-slate-700/60">
              {MODULES.map((mod, i) => (
                <div key={mod.name} className="flex items-center justify-between px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-slate-600 text-xs font-semibold text-slate-500">
                      {i + 1}
                    </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-700 bg-slate-700/20 px-5 py-3">
              <span className="text-xs text-slate-500">Total</span>
              <span className="text-xs font-medium text-slate-300">50–65 min</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-4">
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-5 py-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-400">Before you begin</p>
                <p className="mt-1 text-xs leading-relaxed text-amber-300/70">
                  Find a quiet, uninterrupted space. The assessment takes approximately 50–65
                  minutes. Do not close or refresh the browser once a session has started. There
                  are no right or wrong answers — respond honestly and spontaneously.
                </p>
              </div>
            </div>
          </div>
          ...
          <Link
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-700 py-4 text-base font-semibold text-white transition-colors hover:bg-blue-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900"
          >
            Start Assessment
