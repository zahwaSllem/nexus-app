...
export function RoleBlueprintReview({ blueprint }: RoleBlueprintReviewProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Blueprint header ─────────────────────────────────────────────── */}
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/60 dark:bg-slate-800 dark:shadow-none">
        {/* Top accent bar */}
        ...
        <div className="p-6">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold text-slate-900 dark:text-white">{role_context.role_title}</h2>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={row.label} className="rounded-lg bg-slate-100 px-3 py-2.5 dark:bg-slate-700/40">
                <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">{row.label}</p>
                <p className="mt-0.5 text-sm font-medium text-slate-800 dark:text-slate-200">{row.value}</p>
              </div>
            ))}
          </div>
          ...
          {role_context.environmental_notes && (
            <div className="mt-4 rounded-lg border border-slate-200/80 bg-slate-50 px-4 py-3 dark:border-slate-700/50 dark:bg-slate-900/40">
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Environmental Context</p>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{role_context.environmental_notes}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700/60 dark:bg-slate-800 dark:shadow-none">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Selected Domains & Dimensions</h3>
          <div className="flex gap-2">
            {included_domains.map((d) => (
              <span key={d} className={cn("rounded-full px-2.5 py-0.5 font-mono text-xs font-bold", DOMAIN_COLORS[d] ?? "bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:border-slate-600")}>
                {d}
              </span>
            ))}
          </div>
          ...
          {Object.entries(byDomain).map(([domainId, dims]) => (
            <div key={domainId}>
              <div className="mb-2.5 flex items-center gap-2">
                <span className={cn("rounded-full px-2 py-0.5 font-mono text-xs font-bold", DOMAIN_COLORS[domainId] ?? "bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:border-slate-600")}>
                  {domainId}
                </span>
              ...
              <div className="space-y-2 pl-2">
                {dims.map((dim) => (
                  <div
                    ...
                    className="group rounded-lg border border-slate-200 bg-slate-50/80 px-4 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-100 hover:shadow-md dark:border-slate-700/50 dark:bg-slate-700/20 dark:hover:bg-slate-700/30 dark:hover:shadow-none"
                    style={{
                  ...
                  >
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className="text-sm font-medium text-slate-900 dark:text-white">{dim.dimension_name}</span>
                    </div>
                    <p className="mt-1 text-xs italic text-slate-500 dark:text-slate-400">{dim.selection_rationale}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Exclusion note */}
        <div className="mt-4 rounded-lg border border-slate-200/60 bg-slate-50 px-4 py-3 dark:border-slate-700/40 dark:bg-slate-900/30">
      ...
      {/* ── Context profile ──────────────────────────────────────────────── */}
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700/60 dark:bg-slate-800 dark:shadow-none">
        <h3 className="mb-1 text-sm font-semibold text-slate-900 dark:text-white">Context Profile</h3>
        <p className="mb-4 text-xs text-slate-500">
          These fields define the role environment and derive Domain 6 context-fit indices post-scoring.
        </p>

        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div className="rounded-lg bg-slate-100 px-3 py-2.5 dark:bg-slate-700/40">
            <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Context Name</p>
            <p className="mt-0.5 text-sm font-medium text-slate-800 dark:text-slate-200">{context_profile.context_name}</p>
          </div>
          <div className="rounded-lg bg-slate-100 px-3 py-2.5 dark:bg-slate-700/40">
            <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Job Family</p>
            <p className="mt-0.5 text-sm font-medium text-slate-800 dark:text-slate-200">{context_profile.job_family}</p>
          </div>
          <div className="rounded-lg bg-slate-100 px-3 py-2.5 dark:bg-slate-700/40">
            <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Job Level</p>
            <p className="mt-0.5 text-sm font-medium text-slate-800 dark:text-slate-200">{context_profile.job_level}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {CONTEXT_FIELDS.map(({ key, label, max }) => {
            ...
            return (
              <div key={key} className="rounded-lg bg-slate-50 px-3 py-2.5 dark:bg-slate-700/20">
                ...
                <div className="mt-1.5 flex items-center gap-2">
                  ...
                  <span className="w-5 text-right font-mono text-xs font-bold text-slate-700 dark:text-slate-300">
                    {value}/{max}
                  </span>
                </div>
              </div>
        ...
        {context_profile.success_profile_notes && (
          <div className="mt-4 rounded-lg border border-slate-200/60 bg-slate-50 px-4 py-3 dark:border-slate-700/40 dark:bg-slate-900/30">
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{context_profile.success_profile_notes}</p>
          </div>
        )}
      </div>
        ...
        <div className="flex flex-wrap items-center gap-5">
          ...
          <div className="relative h-[72px] w-[72px] shrink-0">
            <div
              ...
              style={{ background: `conic-gradient(from -90deg, ${bqRingColor} ${bqPct * 3.6}deg, var(--bq-ring-track) ${bqPct * 3.6}deg)` }}
            />
            <div className="absolute inset-[5px] flex flex-col items-center justify-center rounded-full bg-white dark:bg-slate-800">
              <span className={cn("text-xl font-bold tabular-nums leading-none", bqColor)}>{bqPct}</span>
              <span className="mt-0.5 text-[9px] text-slate-400 dark:text-slate-600">/100</span>
            </div>
          </div>

          <div className="flex-1">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Blueprint Quality Score</h3>
          ...
          {BQ_COMPONENTS.map(({ key, label, weight }) => {
            ...
            return (
              <div key={key} className="flex items-center gap-3">
                <span className="w-44 shrink-0 text-xs text-slate-600 dark:text-slate-400">{label}</span>
                <div className="flex flex-1 items-center gap-2">
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                    <div className={cn("h-1.5 rounded-full transition-all duration-700", barColor)} style={{ width: `${pct}%` }} />
                  </div>
                  <span className="w-8 text-right font-mono text-xs text-slate-600 dark:text-slate-400">{pct}%</span>
                </div>
                <span className="w-8 shrink-0 text-right font-mono text-xs text-slate-400 dark:text-slate-600">{weight}</span>
              </div>
            );
          })}
        </div>
